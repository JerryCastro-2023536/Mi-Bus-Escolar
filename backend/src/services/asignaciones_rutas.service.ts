...
export async function listarAsignacionesRutas() {
    const resultado = await pool.query("SELECT * FROM asignaciones_rutas");
...
export async function agregarAsignacionesRutas(p_asignacion_ruta : Asignaciones_Ruta){
    ...
    const consulta = `INSERT INTO asignaciones_rutas(id_estudiante, id_ruta, id_parada_recogida, id_parada_descenso) VALUES ($1, $2, $3, $4)`
...
export async function buscarAsignacionRutaPorId(id : number){
    const valor = [id];
    const resultado = await pool.query("SELECT * FROM asignaciones_rutas WHERE id_asignacion = $1", valor);
    return resultado.rows[0];
}
...
export async function actualizarAsignacionRuta(p_asignacion_ruta : Asignaciones_Ruta, id: number){
    ...
    const consulta = `UPDATE asignaciones_rutas SET id_estudiante = $1, id_ruta = $2, id_parada_recogida = $3, id_parada_descenso = $4 WHERE id_asignacion = $5`;
    const resultado = await pool.query(consulta, valores);
    return resultado.rows[0];
}
...
export async function eliminarAsignacionRuta(id : number) {
    const valor = [id];
    const resultado = await pool.query("DELETE FROM asignaciones_rutas WHERE id_asignacion = $1", valor);
