...
export async function agregarIncidencia(inc: Incidencias){
    try{
        const consulta = "insert into incidencias(id_viaje, id_ruta, id_usuario_reporta, titulo, descripcion, latitud, longitud, fecha_hora, estado) values($1, $2, $3, $4, $5, $6, $7, $8, $9) returning *"
...
export async function actualizarIncidencia(id: number, inc: Incidencias){
    try{
        ...
        const consulta = "update incidencias set id_viaje=$1, id_ruta=$2, id_usuario_reporta=$3, titulo=$4, descripcion=$5, latitud=$6, longitud=$7, fecha_hora=$8, estado=$9 where id_incidencia=$10 returning *"
        const resultado = await pool.query(consulta, values)
